...
import {
  onDragEnd, onDragLeave, onDragOver, onDragStart, onDropCard,
...
const ChangeNameField = function (props:ChangeNameFieldProps) {
  const {
    userState, setUserState, deckName, colonName, taskName, setChanging, firebase,
  } = props;
  const colonObj = colonName.split(' ').join('_');

  const renameDeck = function (name:string, date:string) {
    const newDeck = userState;
    newDeck.decks[deckName]
      .colons[colonObj]
      .tasks[name] = newDeck.decks[deckName].colons[colonObj].tasks[taskName];
  ...
  return (
    <>
      ...
      />
      <button className="taskRedactSubmit" type="submit" onClick={() => renameDeck(inputName, inputDate)}>OK</button>
...
const Task = function (props:TaskProps) {
  const {
    deckName, colonName, taskInfo, userState, setUserState, currentCard, setCurrentCard,
  } = props;
  ...
  const setCompleted = function (firebase:Firebase) {
    ...
    newState.decks[deckName as keyof User]
      .colons[colonObj]
      .tasks[taskInfo.taskName].completed = !checked;
        ...
        >
          {!isChanging ? (
            <>
              ...
              <img src="./redact.png" className="taskRedact" alt="x" onClick={() => { setChanging(!isChanging); }} aria-hidden="true" />
              <img className="taskDelete" alt="delete" src="./delete.png" onClick={() => deleteTask(firebase)} aria-hidden="true" />
              <input className="taskCheckBox" type="checkbox" checked={checked} id={taskInfo.taskName} onChange={() => setCompleted(firebase)} />
